...
int main(void){
	...
	LEDGPIOinit.GPIO_Pin = GPIO_Pin_15;
	...
	LEDGPIOinit.GPIO_Pin = GPIO_Pin_2;
	GPIO_Init(GPIOB, &LEDGPIOinit);	
	...
	while(1){
		...
		if(CalibDelay == 0 && calibGyroDone == 0){
			...
			// Arm with Aux 1
			if(RXcommands[4] > 150 && OkToArm == 250){
				if(Armed == 0){
					Armed = 1;
		...
		if(millis()-last_Time > 100){ // 10Hz
			...
			if(CalibDelay > 0){
				if(CalibDelay%2)GPIO_WriteBit(GPIOA, GPIO_Pin_15, Bit_SET);	
				else GPIO_WriteBit(GPIOA, GPIO_Pin_15, Bit_RESET);	
				CalibDelay--;
			}
			...
			}else if(LiPoVolt < 285) LiPoEmptyWaring++;
			else if(LiPoEmptyWaring > 0) LiPoEmptyWaring--;
		}
