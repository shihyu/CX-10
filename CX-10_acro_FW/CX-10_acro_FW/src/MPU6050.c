...
void ReadMPU(){
	...
	ACC_ORIENTATION( (int16_t)((I2C_rec_Buffer[0]<<8) | I2C_rec_Buffer[1])/16 ,
		(int16_t)((I2C_rec_Buffer[2]<<16) | I2C_rec_Buffer[3])/16,
		(int16_t)((I2C_rec_Buffer[4]<<16) | I2C_rec_Buffer[5])/16);	
	...
	if(calibGyroDone > 0){
		...
		if(calibGyroDone%2)GPIO_WriteBit(GPIOA, GPIO_Pin_15, Bit_RESET);	
		else GPIO_WriteBit(GPIOA, GPIO_Pin_15, Bit_SET);	
...
void init_MPU6050(){
	...
	gpioinitI2C1.GPIO_PuPd = GPIO_PuPd_NOPULL;
